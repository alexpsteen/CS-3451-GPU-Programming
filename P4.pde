...
//    // x, y, z, u, v
//    // xyz 3d coordinates uv are texutre coordinates
//    vertex(-300, 50,  100, 0,0);    // bottom left
//    vertex(-50,  50,  100, 1,0);    // bottom right
//    vertex(-50,  300, 100, 1,1);    // top right
//    vertex(-300, 300, 100, 0,1);    // top left
//  endShape();


  float gridSize = 10;
  for(int y=300; y>50; y-=10){
    for(int x=-300; x<-50; x+=10){
      beginShape();
        texture(gradientTexture);
        vertex(x,           y-gridSize,   100,  (300-abs(x))/250.0,     (y-60)/250.0);      // bottom left
        vertex(x+gridSize,  y-gridSize,   100,  (310-abs(x))/250.0,     (y-60)/250.0);      // bottom right
        vertex(x+gridSize,  y,            100,  (310-abs(x))/250.0,     (y-50)/250.0);        // top right
        vertex(x,           y,            100,  (300-abs(x))/250.0,     (y-50)/250.0);        // top left
      endShape();
    }
  }
